...
function Shell({ children }: { children: React.ReactNode }) {
  const { state, setState } = useStore(); const nav=useNavigate();
...
function Diagnostic() {
  const {setState}=useStore(); const nav=useNavigate(); const [index,setIndex]=useState(0); const [answers,setAnswers]=useState<Record<string,string>>({});
...
function Lesson() {
  const {lessonId}=useParams(); const {state,setState}=useStore(); const nav=useNavigate(); const lesson=LESSONS.find(l=>l.id===lessonId); const [index,setIndex]=useState(0); const [answer,setAnswer]=useState(""); const [feedback,setFeedback]=useState<null|{ok:boolean;text:string}>(null); const [draft,setDraft]=useState(()=>localStorage.getItem(`jangat-draft-${lessonId}`)||"");const [syncState,setSyncState]=useState("enregistré sur cet appareil");
  if(!lesson)return <Navigate to="/dashboard"/>; const step=lesson.steps[index];
  const validate=async()=>{if(step.type==="content"||step.type==="vocabulary"){await recordStep(lesson.id,`${lesson.id}:${index}`,{viewed:true});setFeedback({ok:true,text:"Bien joué. Vous venez de consolider une notion importante."});return} if(step.type==="open"){if(!draft.trim())return;localStorage.setItem(`jangat-draft-${lessonId}`,draft);setSyncState("synchronisation en cours");try{await syncDraft(`${lesson.id}:${index}`,draft,`${lesson.id}:${index}:draft`);setSyncState("synchronisé")}catch(e){setSyncState(e instanceof Error&&e.message==="CONFLICT"?"conflit à résoudre":"échec de synchronisation")}setFeedback({ok:true,text:"Votre première version est conservée. Comparez-la maintenant aux critères proposés."});return}const result=await recordStep(lesson.id,`${lesson.id}:${index}`,answer);const ok=answer===step.correct;setFeedback({ok,text:ok?step.feedbackCorrect||"Exact.":step.feedbackIncorrect||"Ce n’est pas encore acquis. Relisez l’explication et réessayez."});if(!ok&&typeof result==="object")setState(s=>({...s,hearts:Math.max(0,s.hearts-1)}));};
  const next=async()=>{if(index<lesson.steps.length-1){setIndex(index+1);setAnswer("");setFeedback(null)}else{const result=await completeLesson(lesson.id);setState(s=>({...s,completedLessons:Array.from(new Set([...s.completedLessons,lesson.id])),xp:result.xp,hearts:result.hearts,streak:result.streak,lastActivity:new Date().toISOString().slice(0,10),unlockedModules:lesson.moduleId==="module-1"&&lesson.id==="m1-l4"?Array.from(new Set([...s.unlockedModules,"module-2"])):s.unlockedModules}));localStorage.removeItem(`jangat-draft-${lessonId}`);nav(`/unit/${lesson.moduleId}`)}};
  return <Shell><section className="lesson-head"><button onClick={()=>nav(-1)}>×</button><div className="progress"><i style={{width:`${((index+1)/lesson.steps.length)*100}%`}}/></div><span>♥ {state.hearts}</span></section><section className="card lesson-card"><p className="eyebrow">{lesson.title} · Étape {index+1}/{lesson.steps.length}</p><div className="question-title"><h1>{step.title}</h1><button className="speak" onClick={()=>speechService.speak(`${step.title}. ${step.body||step.prompt||""}`,{lang:step.lang||"fr-FR",rate:state.profile.speechRate})}>🔊 Écouter</button></div>{step.body&&<p className="lesson-body">{step.body}</p>}
    ...
    {step.type==="open"&&<><textarea className="open-answer" value={draft} onChange={e=>{setDraft(e.target.value);setSyncState("enregistré sur cet appareil");localStorage.setItem(`jangat-draft-${lessonId}`,e.target.value)}} placeholder="Rédigez votre première version ici…"/><small className="sync-state">{syncState}</small>{feedback&&<div className="rubric"><h3>Auto-évaluation critériée</h3>{(step.criteria||[]).map(c=><label key={c}><span>{c}</span><select><option>Je ne sais pas</option><option>Respecté</option><option>Partiellement respecté</option><option>Non respecté</option></select></label>)}</div>}</>}
    {feedback&&<div className={`feedback ${feedback.ok?"ok":"error"}`} role="status"><strong>{feedback.ok?"✓ Bonne progression":"! Une erreur utile"}</strong><p>{feedback.text}</p><button className="speak" onClick={()=>speechService.speak(feedback.text)}>🔊 Écouter l’explication</button></div>}
...
function Profile({settings=false}:{settings?:boolean}) { const {state,setState}=useStore(); const p=state.profile; return <Shell><section className="course-intro"><p className="eyebrow">{settings?"Préférences":"Votre profil"}</p><h1>{settings?"Réglez votre expérience":"Votre cap professionnel"}</h1></section><section className="card form-grid">
  ...
  {settings&&<><label className="toggle"><input type="checkbox" checked={p.voice} onChange={e=>setState(s=>({...s,profile:{...s.profile,voice:e.target.checked}}))}/> Activer la lecture vocale</label><label>Vitesse<input type="range" min=".7" max="1.4" step=".1" value={p.speechRate} onChange={e=>setState(s=>({...s,profile:{...s.profile,speechRate:Number(e.target.value)}}))}/></label><button onClick={()=>speechService.pause()}>Pause</button><button onClick={()=>speechService.resume()}>Reprendre</button><button onClick={()=>speechService.stop()}>Arrêter</button></>}
...
function AppRoutes(){return <Routes><Route path="/" element={<Navigate to="/login"/>}/><Route path="/login" element={<Auth mode="login"/>}/><Route path="/signup" element={<Auth mode="signup"/>}/><Route path="/forgot-password" element={<Auth mode="forgot"/>}/>{[
