...
export function CompleteCoursePage() {
  ...
  const [track,setTrack]=useState<TrackRecommendation|null>(null);const [showFull,setShowFull]=useState(false);const [notice,setNotice]=useState("");const [error,setError]=useState("");
  ...
  useEffect(()=>{if(!course)return;loadCourseProgress().then(rows=>{const row=rows.find(item=>item.course_key===course.id);if(row?.recommended_track){setTrack(row.recommended_track as TrackRecommendation);setScores(row.positioning_scores as Record<PositioningDimension,number>)}}).catch(caught=>setError(caught instanceof Error?caught.message:String(caught)))},[course]);
